...
export async function downloadSaleOrder(order: SaleOrderDTO) {
  ...
  const fechaPedido = order.orderDate
    ? `Fecha del pedido: ${format(new Date(order.orderDate), "dd/MM/yyyy")}`
    : "";

  const metodoEntrega = order.shipment?.shipmentMethod?.name
    ? `Método de entrega: ${order.shipment.shipmentMethod.name}`
    : "";

  const canalVenta = order.saleChannel
    ? `Canal de venta: ${order.saleChannel}`
    : "";

  // Una sola fila con 3 columnas
  infoBody.push([fechaPedido, metodoEntrega, canalVenta]);
